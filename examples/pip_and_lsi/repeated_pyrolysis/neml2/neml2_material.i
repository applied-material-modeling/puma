...
[Models]
    ...
    [solve_reaction]
        ...
        solver = 'newton'
    []
    # Mass fractions are algebraic functions of the implicitly-solved, bounded
    # conversion alpha (wb0,ws0,wgcp0,phiop0 are spatial gathered inputs). This
    # avoids the explicit rate-integration drift that drove wb<0 and phiop->1
    # (singular composite volume):
    #   wb    = wb0 * (1 - alpha)
    ...
    [model_solver]
        ...
        models = "solve_reaction reaction_coef reaction_rate
                wb0_alpha binder char gas open_pore"
        additional_outputs = 'alpha k'
    []
    ################################### POST PROCESS #################################
    #########
    ############### volume fraction ######
    # wp (product/SiC mass fraction) is a spatial gathered input from MOOSE.
    [V_RVE_post]
